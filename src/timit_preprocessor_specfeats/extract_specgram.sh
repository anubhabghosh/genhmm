...
if [ ${#} != 4  ];then
    echo "Usage: ./specgramfeats_extract.sh [Kaldi's relative path] [TIMIT relative path] [Intermediate folder] [output .scp filename]"
    exit -1
fi
...
if [[ $fname == *".512."* ]]; then
	target=(${fname_trunk//.512})
    ${kaldi}/src/featbin/compute-spectrogram-feats --verbose=2 $options scp:.data/material/$target.wav.scp ark,t,scp:$path/$target.512.ark,$target_file 2> $log
fi
